...
var _ = SIGDescribe("Job", func() {
	...
	ginkgo.It("should not create pods when created in stopped state [StoppedJob]", func() {
		ginkgo.By("Creating a job with stopped=true")
		job := e2ejob.NewTestJob("succeed", "stopped-true-to-false", v1.RestartPolicyNever, true, parallelism, completions, nil, backoffLimit)
		job, err := e2ejob.CreateJob(f.ClientSet, f.Namespace.Name, job)
		framework.ExpectNoError(err, "failed to create job in namespace: %s", f.Namespace.Name)
		...
		ginkgo.By("Checking Job status to observe Stopped state")
		...
		for _, c := range job.Status.Conditions {
			if c.Type == batchv1.JobStopped {
				exists = true
				break
			}
		}
		framework.ExpectEqual(exists, true)
		framework.ExpectNotEqual(job.Status.StopTime, nil)
	...
	ginkgo.It("should delete pods when stopped [StoppedJob]", func() {
		ginkgo.By("Creating a job with stopped=false")
		job := e2ejob.NewTestJob("notTerminate", "stopped-false-to-true", v1.RestartPolicyNever, false, parallelism, completions, nil, backoffLimit)
		job, err := e2ejob.CreateJob(f.ClientSet, f.Namespace.Name, job)
		framework.ExpectNoError(err, "failed to create job in namespace: %s", f.Namespace.Name)
		...
		ginkgo.By("Updating the job with stopped=true")
		job, err = e2ejob.GetJob(f.ClientSet, f.Namespace.Name, job.Name)
		framework.ExpectNoError(err, "failed to retrieve latest job object")
		job.Spec.Stopped = utilpointer.BoolPtr(true)
		job, err = e2ejob.UpdateJob(f.ClientSet, f.Namespace.Name, job)
		framework.ExpectNoError(err, "failed to update job in namespace: %s", f.Namespace.Name)

		ginkgo.By("Ensuring pods are deleted")
		err = e2ejob.WaitForAllJobPodsGone(f.ClientSet, f.Namespace.Name, job.Name)
		framework.ExpectNoError(err, "failed to ensure pods are deleted after stopped=true")
